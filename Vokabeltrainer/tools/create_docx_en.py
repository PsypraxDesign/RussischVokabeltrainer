...
doc.add_heading('2.4 Conjugation and Declension', level=2)
doc.add_paragraph(
    'For verbs and nouns, additional grammar buttons may appear on the flashcard:'
)
p = doc.add_paragraph()
r = p.add_run('Conjugation ')
r.bold = True
p.add_run(
    '(for verbs): Click the button to see the full conjugation table -- '
    'with present, future, past, imperative, participles, and gerund. '
    'Aspect and aspect partner are also shown.'
)
p = doc.add_paragraph()
r = p.add_run('Declension ')
r.bold = True
p.add_run(
    '(for nouns): Click the button to see the declension table -- '
    'with all six cases (Nominative, Genitive, Dative, Accusative, Instrumental, Prepositional) '
    'in singular and plural. Gender and animacy are shown.'
)
doc.add_paragraph(
    'The tables are generated in the Vocabulary Editor (vokabel_editor.html) via AI and automatically '
    'synced with the flashcards. If a conjugated or declined form appears on the card, '
    'it is automatically replaced with the base form (infinitive or nominative singular).'
)
doc.add_paragraph('Press Escape or click the X to close the table.')

doc.add_heading('2.5 The Card List', level=2)
doc.add_paragraph(
...
doc.add_paragraph('Again (Key 1) — Didn\'t know it. The card will appear again soon.', style='List Bullet 2')
doc.add_paragraph('Hard (Key 2) — Barely remembered, had to think hard.', style='List Bullet 2')
doc.add_paragraph('Good (Key 3) — Knew it, but had to think briefly. This is the recommended default rating for correct answers.', style='List Bullet 2')
doc.add_paragraph('Easy (Key 4) — Knew it instantly, without any hesitation.', style='List Bullet 2')
doc.add_paragraph('The app automatically calculates when you should see each card again.', style='List Number')

doc.add_heading('Understanding the Rating Buttons', level=2)
doc.add_paragraph(
    'Below each button, the app shows how many days until the card will appear again with that rating. '
    'These intervals are not fixed — they change based on your learning history with each card.'
)

p = doc.add_paragraph()
r = p.add_run('New cards: Short, closely spaced intervals')
r.bold = True
doc.add_paragraph(
    'When you see a card for the first time, the system has no information about your knowledge yet. '
    'That\'s why the initial intervals are short and close together:'
)
add_table(doc,
    ['Button', 'Interval for a new card'],
    [
        ['Again', '1 day'],
        ['Hard', '1 day'],
        ['Good', '2 days'],
...
doc.add_paragraph(
    'This is completely normal and not a bug! For new cards, it doesn\'t make much difference '
    'whether you press "Hard" or "Again" — the card will come back very soon either way.'
)

p = doc.add_paragraph()
r = p.add_run('Learned cards: Growing, well-differentiated intervals')
r.bold = True
doc.add_paragraph(
    'The more often you successfully review a card, the more the intervals grow and spread apart. '
    'After several reviews with "Good", it might look like this:'
)
add_table(doc,
    ['Button', 'Interval after several reviews'],
    [
        ['Again', '1 day'],
        ['Hard', '3 days'],
        ['Good', '9 days'],
...
doc.add_paragraph(
    'Here you can see the big difference: A well-learned card with "Good" won\'t appear for 9 days, '
    'with "Easy" not for 23 days. With "Again", it\'s almost reset and appears tomorrow.'
)

p = doc.add_paragraph()
r = p.add_run('How do intervals grow?')
r.bold = True
doc.add_paragraph(
    'The FSRS system remembers how often and how well you rated each card. '
    'From this, it calculates an individual stability value:'
)
doc.add_paragraph('Each time you rate a card "Good" or "Easy", stability increases — the interval gets longer.', style='List Bullet')
doc.add_paragraph('With "Hard", the interval grows more slowly.', style='List Bullet')
doc.add_paragraph('With "Again", stability is nearly reset — the card comes back like an almost-new card.', style='List Bullet')

p = doc.add_paragraph()
r = p.add_run('Practical tip: ')
r.bold = True
p.add_run(
    'Rate most cards "Good" (Key 3). Use "Easy" only for words you truly know in your sleep. '
    '"Hard" is appropriate when you were unsure but still got it right. '
    '"Again" is for when you couldn\'t recall the answer at all.'
)

doc.add_heading('What do "Due", "New", and "Done" mean?', level=2)
doc.add_paragraph('In the status bar at the bottom you\'ll see three numbers:')
doc.add_paragraph('Due (red) — Cards you\'ve learned before that are scheduled for review today. These have priority.', style='List Bullet')
doc.add_paragraph('New (blue) — Cards you\'ve never rated before. A maximum of 20 new cards are introduced per session.', style='List Bullet')
doc.add_paragraph('Done (green/gray) — Cards you\'ve already reviewed in this session.', style='List Bullet')
doc.add_paragraph(
    'When all cards are learned and none are due, the app shows the message '
    '"No cards due and no new cards. Come back later!" '
    'This means: You\'re done for today! Come back tomorrow (or whenever the next cards are due) to continue learning.'
)
...
# --- 9 ---
doc.add_heading('9. Do I need an API key?', level=1)
doc.add_paragraph(
    'For the vocabulary trainer (index.html) itself: no. Learning flashcards, reading texts aloud, '
    'spaced repetition, viewing conjugation and declension tables, images, export — '
    'everything works completely offline without an API key.'
)
doc.add_paragraph(
    'An Anthropic Claude API key is required only in the vocabulary editor (vokabel_editor.html), '
    'and even there only for the following AI features:'
)
add_table(doc,
    ['Feature', 'Purpose'],
    [
        ['Word types', 'Automatically determines whether a word is a verb, noun, adjective, …'],
        ['Conjugation', 'Generates complete conjugation tables for verbs'],
        ['Declension', 'Generates declension tables for nouns (all 6 cases, sg./pl.)'],
        ['Stress', 'Marks the stressed syllable (used by the pronunciation trainer)'],
        ['Normalize', 'Converts vocabulary to dictionary form'],
        ['AI vocab filter', 'Filters out junk, duplicates and Latin script when importing texts'],
    ]
)
doc.add_paragraph(
    'All AI features only generate missing data — existing conjugations, declensions, etc. '
    'are skipped, keeping API usage minimal.'
)
doc.add_paragraph(
    'If you only want to learn and do not need to enrich your own vocabulary via AI, no setup '
    'is required. For details on API key configuration, see Gebrauchsanweisung_Editor.md.'
)
doc.add_heading('Pronunciation trainer: Microsoft Azure', level=2)
doc.add_paragraph(
    'The pronunciation trainer (aussprache_trainer.html) additionally uses the Microsoft Azure '
    'Speech Service to evaluate your pronunciation at the phoneme level. This requires a '
    'separate Azure key — not the Anthropic key.'
)
doc.add_paragraph('Provider: Microsoft Azure (the free F0 tier is sufficient)', style='List Bullet')
doc.add_paragraph('Required: API key + region (e.g. germanywestcentral)', style='List Bullet')
doc.add_paragraph('Entered once in the pronunciation trainer setup screen, then stored in the browser', style='List Bullet')
doc.add_paragraph(
    'Only needed if you want to record and have your pronunciation evaluated. Normal read-aloud '
    'of vocabulary (TTS in the vocabulary trainer) does not require an Azure key.',
    style='List Bullet'
)

# --- 10 ---
doc.add_heading('10. Tips and Notes', level=1)
...
# --- 11 ---
doc.add_heading('11. File Format Summary', level=1)
add_table(doc,
...
output_path = r'C:\Users\klaus\Desktop\Russisch\Vokabeltrainer\docs\User_Guide_EN.docx'
